...
async function getWeatherNWS(lat: number, lon: number): Promise<string | null> {
  try {
    const pointRes = await fetch(`https://api.weather.gov/points/${lat},${lon}`, { headers: { 'User-Agent': 'TARS-AI/1.0', Accept: 'application/json' }, signal: AbortSignal.timeout(6000) })
    ...
    const days = (forecast?.properties?.periods ?? []).filter((p: { isDaytime: boolean }) => p.isDaytime).slice(0, 3)
      .map((p: { name: string; temperature: number; shortForecast: string; windSpeed: string }) => ({ day: p.name, condition: p.shortForecast, high_f: p.temperature, wind: p.windSpeed }))
    return JSON.stringify({ source: 'NWS', condition: current.shortForecast, temp_f: current.temperature, wind: current.windSpeed + ' ' + current.windDirection, humidity: current.relativeHumidity?.value ? current.relativeHumidity.value + '%' : 'N/A', forecast: days }, null, 2)
  } catch { return null }
}
...
async function getWeather(location: string): Promise<string> {
  ...
  if (!simpleName.toLowerCase().includes('beverly')) {
    try {
      const geoRes = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(simpleName)}&count=1&language=en&format=json`, { signal: AbortSignal.timeout(5000) })
  ...
  try {
    const wxRes = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph&forecast_days=3&timezone=auto`, { signal: AbortSignal.timeout(5000) })
    if (!wxRes.ok) throw new Error(`Open-Meteo ${wxRes.status}`)
    const wx = await wxRes.json()
    const c = wx.current; const d = wx.daily
    return JSON.stringify({ source: 'Open-Meteo', location: placeName, condition: WMO_CODES[c.weather_code], temp_f: Math.round(c.temperature_2m), feels_like_f: Math.round(c.apparent_temperature), humidity: c.relative_humidity_2m + '%', wind_mph: Math.round(c.wind_speed_10m), forecast: d.time?.map((date: string, i: number) => ({ date, condition: WMO_CODES[d.weather_code[i]], high_f: Math.round(d.temperature_2m_max[i]), low_f: Math.round(d.temperature_2m_min[i]) })) }, null, 2)
  } catch (err) { return `Weather unavailable: ${err}` }
}
...
export const TARS_TOOLS: Anthropic.Tool[] = [
  ...
  { name: 'web_search', description: 'Search the web for news, research, leads, or any information.', input_schema: { type: 'object' as const, properties: { query: { type: 'string' } }, required: ['query'] } },
  ...
  { name: 'gmail_read', description: 'Read and summarize a specific email by ID.', input_schema: { type: 'object' as const, properties: { message_id: { type: 'string' } }, required: ['message_id'] } },
  ...
  { name: 'calendar_list', description: 'List upcoming calendar events for the next N days.', input_schema: { type: 'object' as const, properties: { days: { type: 'number' } } } },
  { name: 'calendar_create', description: 'Create a calendar event.', input_schema: { type: 'object' as const, properties: { title: { type: 'string' }, start_datetime: { type: 'string', description: 'ISO 8601 format, e.g. 2026-05-18T14:00:00' }, end_datetime: { type: 'string' }, description: { type: 'string' }, location: { type: 'string' } }, required: ['title', 'start_datetime', 'end_datetime'] } },
]
...
export async function executeTool(name: string, input: Record<string, unknown>, accessToken: string | undefined): Promise<string> {
  try {
    switch (name) {
      ...
      case 'calendar_list': {
        if (!accessToken) return 'Calendar not connected. Sign out and sign back in.'
        const events = await listEvents(accessToken, (input.days as number) ?? 7)
        return events.length ? JSON.stringify(events, null, 2) : 'No upcoming events.'
      }
      case 'calendar_create': {
        if (!accessToken) return 'Calendar not connected. Sign out and sign back in.'
        const event = await createEvent(accessToken, input.title as string, input.start_datetime as string, input.end_datetime as string, input.description as string | undefined, input.location as string | undefined)
        return `Event created: ${event.link}`
      }
