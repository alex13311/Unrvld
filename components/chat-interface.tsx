  ...
  async function send(text: string) {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: next, location }),
      })
      ...
      <header className="flex items-center justify-between px-6 py-4 shrink-0" style={{ borderBottom: `1px solid ${amber}0.12)` }}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => signOut({ callbackUrl: '/' })} className="transition" style={{ color: `${amber}0.25)` }} onMouseEnter={(e) => (e.currentTarget.style.color = `${amber}0.7)`)} onMouseLeave={(e) => (e.currentTarget.style.color = `${amber}0.25)`)} title="Sign out">
              <LogOut size={14} />
            </button>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-3xl flex flex-col gap-6">
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-lg">
                {SUGGESTIONS.map((s) => (
                  <button key={s} onClick={() => send(s)} className="px-4 py-2 text-xs transition font-mono" style={{ border: `1px solid ${amber}0.1)`, color: `${amber}0.4)`, borderRadius: '2px' }}
                    ...
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = `${amber}0.1)`; e.currentTarget.style.color = `${amber}0.4)`; e.currentTarget.style.background = 'transparent' }}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex fade-up ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className="max-w-[80%] px-4 py-3 text-sm leading-[1.9] whitespace-pre-wrap font-mono"
                style={msg.role === 'user'
      ...
      <div className="shrink-0 px-6 py-4" style={{ borderTop: `1px solid ${amber}0.1)` }}>
        <div className="mx-auto max-w-3xl flex items-end gap-3">
          <textarea ref={textareaRef} value={input}
            ...
            onKeyDown={handleKey} placeholder="Speak to TARS…" rows={1} disabled={loading}
        ...
        <p className="mt-2 text-center font-mono text-[10px] uppercase tracking-[0.3em]" style={{ color: `${amber}0.18)` }}>Enter to send · Shift+Enter for new line</p>
      </div>
    </div>
  )
}
